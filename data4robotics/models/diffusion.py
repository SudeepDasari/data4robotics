...
class TransformerBlock(nn.Module):
    def __init__(self, dim, cond_dim, dropout=0.1, layer_norm=True, num_heads=8):
        ...
        self.norm_map = nn.LayerNorm(dim) if layer_norm \
                        else nn.Identity()
        
        self.proj_out = nn.Sequential(nn.Linear(dim, dim), 
                                      nn.ReLU(inplace=True))
        self.norm_out = nn.LayerNorm(dim) if layer_norm \
                        else nn.Identity()
...
class NoiseNetwork(nn.Module):
    def __init__(self, ac_dim, global_cond_dim, ac_chunk, time_dim=256, hidden_dim=512,
        ...
        self.ac_proj = nn.Sequential(nn.Linear(ac_dim, ac_dim),
                                     nn.ReLU(inplace=True),
                                     nn.Linear(ac_dim, hidden_dim))

        # transformer blocks
        self.blocks = nn.ModuleList([TransformerBlock(hidden_dim, cond_dim, dropout) 
